...
test:  $(ENV)
	- $(ENV_BIN)/tox -vv
	$(ENV_BIN)/gunicorn --paste syncserver/tests.ini 2> /dev/null & SERVER_PID=$$!; \
	sleep 2; \
	$(ENV_BIN)/python -m syncstorage.tests.functional.test_storage \
